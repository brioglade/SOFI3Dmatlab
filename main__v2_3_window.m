...
%Edit Nov 6, 2018: (v2.3) 1. Update export_seismograph to
%                           export_seismograph2 for accomdating S-wave correction
%                         2. Add new function: define core.
% PLEASE USE SI UNITS %
...
source.location = [];
source.frequency = 1e6 ;     %[Hz] source frequency  
source.amplitude = 1e-6;    %[m] source amplitude
...
% Define the number of processing cores
[core] = define_core(core, SSH_machine_num, cube);
display('Finished define_core')
% Write source file
[source] = create_source(path,cube,source);
display('Finished create_source')
% Write receiver file
[receiver] = create_receiver(path,cube,receiver,simulation);
...
% Pause time for memory flushing 
pause(15)                      %pause 15 seconds 

% Export Seismogram and Plot output
[plt_params] = export_seismograph2(cube, path, fd, receiver, simulation);
